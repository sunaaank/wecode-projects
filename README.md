...
### 🎯 학습 목표
- 4주 기간 내에 이커머스 하이브리드앱 개발
- 자바스크립트 비동기 개념을 이해하고 적합한 HTTP 요청 메소드를 사용해 API 통신을 구현한다.
- 필요한 개념공부를 직접 찾아 하고, 공식문서를 찾아 읽고 적용하는 것에 익숙해진다.
- recoil을 익혀 전역상태관리를 도입한다.
- Framework7, Tailwind,  라이브러리 사용법을 익혀 신속하게 프론트엔드를 구현한다.
- RDBMS 개념, ERD 사용법을 익히고 루비, 루비온레일즈를 활용해 백엔드를 구현한다.
- postman을 활용해 프백 통신을 원활하게 하는 법을 익힌다.

### 주요 문제
- `문제`: 전역 상태관리할 데이터(장바구니에 담긴 상품 데이터, 장바구니 상품 유무 체크 등)의 api 통신 위치, 상태 변경에 따른 의존성 배열 처리를 원활히 하기 위해 데이터 형태와 종류를 고치는 작업을 몇차례 진행하였습니다. 
- `해결`: 유저가 로그인한 첫 화면에서 장바구니 상태 관련 데이터를 받아오도록 처리했습니다.
- `회고`: 기능구현에 앞서 앱의 전체적인 상태 관리 구조와 흐름 설계가 중요하다는 걸 알게 되었습니다.
...
### ✨ 새롭게 알게 된 점
- 사이트 구조 분석을 통해 각 페이지에 필요한 기능과 이벤트, 재사용할 컴포넌트를 구분하는 눈이 생겼습니다.
- 컴포넌트 재사용, 함수 재사용을 좀 더 이해하고 활용하는 코드를 작성했습니다.
- 리액트 클래스/함수 컴포넌트의 라이프사이클 및 사용상 차이점을 경험했습니다.
- 카카오 서버를 경유하는 인증 방식에서의 데이터 송수신 과정을 체험했습니다.(OAuth2.0)
- state의 불변성 유지를 위해 spread 연산자를 사용한 객체의 얕은 복사 방법을 이해했습니다.
- git rebase를 통해 commit 메세지를 깔끔하게 관리할 수 있었습니다.
